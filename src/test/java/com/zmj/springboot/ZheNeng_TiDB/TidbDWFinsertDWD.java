...
/**
 * TiDB库将DWF层的表数据插入到对应的DWD表中
 * 前提是 DWD表与DWD表结构相同，只表名字不同
 */
public class TidbDWFinsertDWD {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String tableNamePart = "SETT_EBANKINSTRUCT";

        String querySQL = "SELECT TABLE_NAME,COLUMN_NAME FROM information_schema.columns "
                 + "WHERE table_schema = '" + databaseName +"' "
                //+ " and table_name in ('DWF_SAP_STG_SAP_VOUCHER_ITEM') "
        ...
        for (Map.Entry<String, StringTwo> entry : columnDataMap.entrySet()) {
            ...
            sqlResult.append("insert into ").append(replaceDbName).append(".").append(tableName.replaceFirst(currTablePrefix,replaceTablePrefix))
                    .append("(").append(two.getA().substring(0,two.getA().length()-1)).append(") select ")
                    .append(two.getB().substring(0,two.getB().length()-1)).append(" from ").append(databaseName).append(".").append(tableName).append(" g where g.DATA_DT = '${V_DATE}';\n\n");
        }
        //System.out.println(sqlResult);
        try {
            MyFileUtils.print2File(sqlResult,"D:\\MyOutputFile\\TiDB_DWFinsertDWD_v20250901.sql",false);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
